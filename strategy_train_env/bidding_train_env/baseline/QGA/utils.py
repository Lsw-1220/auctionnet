...
class EpisodeReplayBuffer(Dataset):
    def __init__(self, state_dim, act_dim, data_path='data/autoBidding_aigb_track_data_trajectory_data.csv', max_ep_len=48, scale=50, K=10, sparse_data=False):
        ...
        data_paths = [data_path] if isinstance(data_path, (str, os.PathLike)) else list(data_path)
        if not data_paths:
            raise ValueError('data_path must contain at least one CSV file')
        missing_paths = [str(path) for path in data_paths if not os.path.isfile(path)]
        if missing_paths:
            raise FileNotFoundError(f'Training data files not found: {missing_paths}')

        print(f'loading data from {len(data_paths)} file(s): {data_paths}...')
        training_data = pd.concat(
            (pd.read_csv(path) for path in data_paths),
            axis=0,
            ignore_index=True,
        )
