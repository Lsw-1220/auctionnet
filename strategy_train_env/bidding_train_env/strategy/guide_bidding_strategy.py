...
class GUIDEStrategy(BaseBiddingStrategy):
    """
    GUIDE (Generative Auto-Bidding with Unified Modeling and Exploration) Strategy.
    ...
    def __init__(self, budget=100, name="GUIDE-Strategy", cpa=2, category=1,
        ...
        if critic_path is None:
            critic_path = os.path.join(_STRATEGY_TRAIN_ENV_DIR, "saved_model", "GUIDE", "GUIDE_critic_inverse.pt")
        if idm_path is None:
    ...
    def bidding(self, timeStepIndex, pValues, pValueSigmas, historyPValueInfo, historyBid,
        ...
        Compatible with AuctionNet's bidding interface.
        """
        ...
        # Core: DT + IDM action selection with Critic
        alpha = self.model.take_action_inverse(
            self.test_state,
            actual_executed_action=None,  # AuctionNet interface doesn't provide executed action
