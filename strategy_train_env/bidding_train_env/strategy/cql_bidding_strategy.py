...
class CqlBiddingStrategy(BaseBiddingStrategy):
    ...
    def __init__(self, budget=100, name="Cql-PlayerStrategy", cpa=2, category=1, model_dir=None):
        super().__init__(budget, name, cpa, category)

        file_name = os.path.dirname(os.path.realpath(__file__))
        dir_name = os.path.dirname(file_name)
        dir_name = os.path.dirname(dir_name)
        model_dir = model_dir or os.path.join(dir_name, "saved_model", "CQLtest")
        model_path = os.path.join(model_dir, "cql_model.pth")
